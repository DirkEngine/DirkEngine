...
pub(crate) enum Garbage {
    ...
    Surface(vk::SurfaceKHR, std::sync::Arc<dyn crate::SurfaceTarget>),
    Swapchain {
        raw: vk::SwapchainKHR,
...
pub(crate) struct Context {
    ...
    garbage: Mutex<crate::retirement::RetirementQueue<Garbage>>,
...
impl Context {
    ...
    pub(crate) fn new(info: &RhiCreateInfo<'_>) -> Result<Arc<Self>> {
        ...
        Ok(Arc::new(Self {
            ...
            garbage: Mutex::new(crate::retirement::RetirementQueue::default()),
            debug_messenger,
        }))
    }
    ...
    // Retire into the current cycle; the RHI collects sealed cycles after GPU completion.
    pub(crate) fn retire(&self, value: Garbage) {
        self.garbage.lock().push(value);
    }

    pub(crate) fn collect_garbage(&self) {
        let pending = self.garbage.lock().collect();
        for value in pending {
            self.destroy(value);
        }
    }
    pub(crate) fn seal_garbage(&self) {
        self.garbage.lock().seal();
    }
    pub(crate) fn collect_all_garbage(&self) {
        let pending: Vec<_> = self.garbage.lock().drain().collect();
        for value in pending {
            self.destroy(value);
        }
    }
    ...
    fn destroy(&self, garbage: Garbage) {
        unsafe {
            match garbage {
                ...
                Garbage::Surface(raw, _target) => self.surface_loader.destroy_surface(raw, None),
                Garbage::Swapchain {
                    raw,
                    views,
                    semaphores,
