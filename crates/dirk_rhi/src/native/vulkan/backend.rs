...
use crate::{
    ...
    GraphicsPipelineDesc, ImageDesc, ImageUsages, ImageViewDesc, InvalidResourceKind as Ir,
    NativeCommandBuffer, PipelineLayoutDesc, QueueType, Result, RhiCreateInfo, SampleCounts,
...
impl crate::Api for VulkanBackend {
...
unsafe impl Backend for VulkanBackend {
    ...
    fn format_capabilities(&self, format: TextureFormat) -> FormatCapabilities {
        ...
        {
            if filterable {
                crate::BlitSupport::Linear
            } else {
                crate::BlitSupport::Nearest
            }
        } else {
            crate::BlitSupport::None
        };
        FormatCapabilities {
            usages,
            filterable,
