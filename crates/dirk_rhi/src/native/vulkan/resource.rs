...
pub struct VulkanBuffer(BufferInner);
...
unsafe impl NativeBuffer for VulkanBuffer {
    fn size(&self) -> u64 {
        self.0.size
    }
    ...
    unsafe fn write(&self, offset: u64, data: &[u8]) -> Result<()> {
        ...
        let allocation = self.0.allocation.as_ref().ok_or(Ir::BadState)?;
...
pub struct VulkanImageView(ImageViewInner);
...
impl VulkanImageView {
    ...
    pub(crate) fn aspects(&self) -> crate::ImageAspects {
        self.0.aspects
    }
...
pub struct VulkanSampler(SamplerInner);
...
pub struct VulkanShader(ShaderInner);
...
pub struct VulkanBindGroupLayout(BindGroupLayoutInner);
...
pub struct VulkanBindGroup(BindGroupInner);
...
pub struct VulkanPipelineLayout(PipelineLayoutInner);
...
impl VulkanPipelineLayout {
    pub(crate) fn create(
        context: &Arc<Context>,
        desc: &crate::PipelineLayoutDesc<'_, VulkanBackend>,
    ) -> Result<Self> {
        if desc
            .bind_group_layouts
            .iter()
...
pub struct VulkanGraphicsPipeline(GraphicsPipelineInner);
...
unsafe impl NativeFence for VulkanFence {
