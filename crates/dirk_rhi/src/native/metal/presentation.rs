...
use crate::{
    ColorSpace, Extent3d, ImageUsages, InvalidResourceKind as Ir, NativeSurfaceFrame,
    NativeSwapchain, Result, SurfaceCreateInfo, SurfaceFormat, SurfaceStatus, SwapchainDesc,
    TextureFormat,
};
...
impl Drop for SurfaceInner {
    fn drop(&mut self) {
        self.context.retire(super::backend::Garbage::Surface(
            self.layer.clone(),
            self.target.clone(),
        ));
    }
...
impl MetalSurface {
    pub(crate) fn create(context: &Arc<Context>, info: &SurfaceCreateInfo) -> Result<Self> {
        let window = info.window_handle().map_err(|error| {
            crate::Error::Backend(anyhow::anyhow!("window handle is unavailable: {error:?}"))
        })?;
...
impl MetalSwapchain {
    pub(crate) fn create(
        ...
        desc.surface
            .0
            .layer
            .set_pixel_format(super::convert::format(format.texture));
...
unsafe impl NativeSwapchain<MetalBackend> for MetalSwapchain {
...
pub struct MetalSurfaceFrame {
    ...
    resources: Option<(MetalImage, MetalImageView)>,
    format: SurfaceFormat,
    extent: Extent3d,
...
unsafe impl NativeSurfaceFrame<MetalBackend> for MetalSurfaceFrame {
    fn take_resources(&mut self) -> Result<(MetalImage, MetalImageView)> {
