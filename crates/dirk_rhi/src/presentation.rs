...
pub struct SwapchainDesc<'a, B: Api = crate::Rhi> {
    /// Debug label.
    pub label: &'a str,
...
pub unsafe trait NativeSwapchain<B: Api> {
...
/// Implementations must preserve native object lifetimes and obey the shared
/// [`crate::Backend`] contract. Native operations are called with validated inputs.
pub unsafe trait NativeSurfaceFrame<B: Api> {
    /// Moves the acquisition's unique image and view into the portable frame.
    fn take_resources(&mut self) -> Result<(B::Image, B::ImageView)>;
    /// Image format.
    fn format(&self) -> SurfaceFormat;
    /// Image dimensions.
    fn extent(&self) -> Extent3d;
