...
mod tests {
    ...
    fn presentation_encodes_midtones_once_for_srgb_and_unorm_targets() -> anyhow::Result<()> {
        ...
        for source_format in [TextureFormat::Rgba8Unorm, TextureFormat::Rgba8Srgb] {
            for target_format in [TextureFormat::Rgba8Unorm, TextureFormat::Rgba8Srgb] {
                presenter.check_midtone(&mut rhi, source_format, target_format)?;
            }
        }
        rhi.wait_idle()?;
        assert_eq!(
            ...
            0,
            "native presentation validation errors"
        );
        Ok(())
    }
    ...
    impl Presenter {
        fn check_midtone(
            ...
            self.set_target_format(rhi, target_format)?;
            ...
            graph
                .add_pass("linear midtone")
                .write_color_attachment(src, AttachmentInfo::clear_color(0.18, 0.18, 0.18, 1.0));
            graph
            ...
            for row in pixels.chunks_exact(layout.bytes_per_row.get() as usize) {
                for pixel in row[..16].as_chunks::<4>().0 {
