...
mod resources;
use resources::{RecordedCommands, Rhi, SurfaceFrame};

mod proxy;
use proxy::{scene::SceneManager, systems::RendererSystem};

mod render_commands;
use render_commands::RenderChanges;
...
use dirk_render_utils::graph as frame_graph;
...
impl Renderer {
    ...
    pub fn init(
        ...
        let frames = std::array::from_fn(|_| Frame { completion: None });
        let models = models::ModelRegistry::new(&mut rhi, event_manager)?;
        let scene_manager = SceneManager::init(&rhi, properties)?;
        #[cfg(feature = "editor")]
        let egui = EguiState::new(&rhi, properties)?;
        #[cfg(feature = "editor")]
        let viewport_editor = ViewportEditor::new(player_input_sender);
    ...
    fn universe_builder(&mut self) -> UniverseBuilder {
        Universe::builder().with_system(RendererSystem::new(self.changes.clone()))
    }
    ...
    fn tick(&mut self, _delta_time: f64, universe: &Universe) -> Result<()> {
        ...
        let deltas = self
            .changes
            .extract(universe, self.scene_manager.entities());
        self.scene_manager.apply(deltas);
        self.scene_manager.reconcile_views(&mut self.viewports);
        ...
        for event in platform_events {
            match event {
                PlatformEvent::WindowCreated { id } => {
                    ...
                    let window = window::Window::build(&self.rhi, plat_window)?;
    ...
    fn end_frame(&mut self) -> Result<()> {
        ...
        #[cfg(feature = "editor")]
        {
            ...
            self.viewport_editor.apply_resize_requests(
                &self.rhi,
                &mut self.viewports,
                &mut self.egui,
            )?;
        ...
        for player in rendered_players {
            if let Some(viewport) = self.viewports.get_mut(&player) {
                viewport.mark_render_submitted();
            }
        }
    ...
    fn record_viewport_graph(
        ...
        for viewport in self.viewports.values() {
            ...
            let Some(_) = viewport.camera else {
                continue;
            };
            ...
            self.scene_manager
                .render(&mut graph, &self.models, viewport, output, frame_index)?;
        }
        ...
        let mut cmd = self.rhi.create_encoder("viewport render graph")?;
        // SAFETY: frame-slot waits and graph declarations cover these renderer-owned resources.
        unsafe {
            graph.run(&self.rhi, &mut cmd)?;
        }
        let cmd = cmd.finish()?;
    ...
    fn record_presentation_graph(
        ...
        >,
    ) -> Result<Option<RecordedCommands>> {
        #[cfg(not(feature = "editor"))]
        let _ = frame_index;
        if targets.is_empty() {
            return Ok(None);
        ...
        if let Some((swapchain, extent)) = egui_target {
            ...
            for viewport in self.viewports.values() {
                let rendered = viewport_submission
                    .is_some_and(|s| s.rendered_players.contains(&viewport.player()));
                if rendered || viewport.has_rendered() {
                    sampled.push(graph.import_texture(if rendered {
                        viewport.import_after_render()
                    } else {
                        viewport.import()
                    })?);
            ...
            self.egui
                .add_pass(&mut graph, swapchain, extent, frame_index, &sampled)?;
        }
    ...
    fn update_non_editor_presentation(&mut self) -> Result<()> {
        ...
        for (window_id, player) in assignments {
            ...
            if let Some(viewport) = self.viewports.get_mut(&player) {
                viewport.resize(&self.rhi, extent)?;
            }
        }
    ...
    fn submit_frame(
        ...
        // SAFETY: commands and acquisitions belong to this cycle; transfer producers have submitted.
        self.frames[frame_index].completion = Some(unsafe {
            self.rhi.queue::<dirk_rhi::Graphics>().submit(
                command_buffers,
                &dirk_rhi::SubmitInfo {
...
impl Drop for Renderer {
    fn drop(&mut self) {
        if let Err(error) = self.rhi.wait_idle() {
