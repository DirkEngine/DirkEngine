...
#[derive(Debug, Error)]
pub enum Error {
    /// Error while building or executing rendering utilities.
    #[error(transparent)]
    Rendering(#[from] anyhow::Error),
    /// Error produced by the render hardware interface.
    #[error("RHI error: {0}")]
    Rhi(#[from] dirk_rhi::Error),

    /// Error produced by the [`dirk_platform`] crate.
    #[error("platform error: {0}")]
    Platform(#[from] dirk_platform::Error),
    /// Error produced by the [`dirk_assets`] crate.
