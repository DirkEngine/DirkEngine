...
The original stack showed why the renderer's `build.rs` computes Metal buffer/texture/sampler
indices during SPIR-V translation. `MetalPipelineLayout` independently computes
the runtime binding indices. This agreement now uses the shared BindingMap described below. Coordinate conversion
...
[backend conformance plan](backend-conformance.md). Compilation alone does
